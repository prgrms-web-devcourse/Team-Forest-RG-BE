...
public class DataInitializer {
	...
	static final List<Integer> addressCodes = List.of(11, 21, 22, 23, 24, 25, 26, 29, 31, 32, 33, 34, 35, 36, 37, 38,
		...
		11160, 11170, 11180, 11190, 11200, 11210, 11220, 11230, 11240, 11250, 21010, 21020, 21030, 21040, 21050, 21060,
		21070, 21080, 21090, 21100, 21110, 21120, 21130, 21140, 21150, 21310, 22010, 22020, 22030, 22040, 22050, 22060,
		22070, 22310, 23010, 23020, 23040, 23050, 23060, 23070, 23080, 23090, 23310, 23320, 24010, 24020, 24030, 24040,
		24050, 25010, 25020, 25030, 25040, 25050, 26010, 26020, 26030, 26040, 26310, 29010, 31010, 31011, 31012, 31013,
		31014, 31020, 31021, 31022, 31023, 31030, 31040, 31041, 31042, 31050, 31060, 31070, 31080, 31090, 31091, 31092,
		31100, 31101, 31103, 31104, 31110, 31120, 31130, 31140, 31150, 31160, 31170, 31180, 31190, 31191, 31192, 31193,
		31200, 31210, 31220, 31230, 31240, 31250, 31260, 31270, 31280, 31350, 31370, 31380, 32010, 32020, 32030, 32040,
		32050, 32060, 32070, 32310, 32320, 32330, 32340, 32350, 32360, 32370, 32380, 32390, 32400, 32410, 33020, 33030,
		33040, 33041, 33042, 33043, 33044, 33320, 33330, 33340, 33350, 33360, 33370, 33380, 33390, 34010, 34011, 34012,
		34020, 34030, 34040, 34050, 34060, 34070, 34080, 34310, 34330, 34340, 34350, 34360, 34370, 34380, 35010, 35011,
		35012, 35020, 35030, 35040, 35050, 35060, 35310, 35320, 35330, 35340, 35350, 35360, 35370, 35380, 36010, 36020,
		36030, 36040, 36060, 36310, 36320, 36330, 36350, 36360, 36370, 36380, 36390, 36400, 36410, 36420, 36430, 36440,
		36450, 36460, 36470, 36480, 37010, 37011, 37012, 37020, 37030, 37040, 37050, 37060, 37070, 37080, 37090, 37100,
	...
	private static User createUser(UserRepository userRepository, String username) {
		int ridingStartYear = RANDOM.nextInt(1990, 2022);
		int randomSeed = RANDOM.nextInt(3);
		RidingLevel level = BEGINNER;
		switch (randomSeed) {
		...
		switch (randomSeed) {
			case 0: intro.append("자전거가 너무 좋아! "); break;
			case 1: intro.append("안녕하세요! "); break;
			case 2: intro.append("반갑습니다. "); break;
		}
		randomSeed = RANDOM.nextInt(20, 60);
		intro.append(randomSeed).append("세 ");

		randomSeed = RANDOM.nextInt(3);
		switch (randomSeed) {
			case 0: intro.append("취미로 자전거를 즐기고 있습니다!"); break;
			case 1: intro.append("아마추어 자전거 선수 입니다!"); break;
			case 2: intro.append("요즘 라이딩에 푹 빠진 사람입니다!"); break;
		}

		var user = User.builder()
			.nickname(new Nickname(username))
			.manner(Manner.create())
			.profile(new RiderProfile(ridingStartYear, level))
			.introduction(new Introduction(intro.toString()))
			.phoneNumber("010" + nextInt(1000, 9999) + nextInt(1000, 9999))
			.build();
		return user = userRepository.save(user);
	}

	private void insertRidingPost(ConfigurableApplicationContext context, String bikeType, int addressCode,
		RidingLevel ridingLevel, User leader) {
		...
		var tempThumbnailImage = new TemporaryImage("RG_Logo.png",
		...
		var tempSubImage = new TemporaryImage("rogo2.png",
		...
		var mainCreateCommand = RidingMainSaveCommand.builder()
			...
			.estimatedTime(nextInt(1, 6) + "시간")
			.ridingDate(LocalDateTime.now().plusDays(nextLong(1, 20)))
			.fee(nextBoolean() ? 10000 : 0)
			.addressCode(new AddressCode(addressCode))
			.routes(routes)
			.build();
		...
		var createCommand = new RidingSaveCommand(thumbnailId, mainCreateCommand,
			new RidingParticipantSaveCommand(minParticipant, maxParticipant),
			new RidingConditionSaveCommand(ridingLevel.getLevelName(), List.of(bikeType)), List.of(subCommand));
