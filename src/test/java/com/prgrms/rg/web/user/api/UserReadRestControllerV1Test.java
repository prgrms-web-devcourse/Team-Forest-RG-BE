...
class UserReadRestControllerV1Test {
	...
	static FieldDescriptor[] ridingDescription = new FieldDescriptor[]{
		fieldWithPath("id").description("라이딩 아이디"),
		fieldWithPath("title").description("라이딩 제목"),
		fieldWithPath("thumbnail").description("라이딩 썸네일 사진"),
		fieldWithPath("ridingLevel").description("라이딩 난이도"),
		fieldWithPath("ridingDate").description("라이딩 일시"),
		fieldWithPath("departurePosition").description("라이딩 출발 지점"),
		fieldWithPath("departurePosition.lng").description("라이딩 출발 지점 위도"),
		fieldWithPath("departurePosition.lat").description("라이딩 출발 지점 경도"),
		fieldWithPath("zone").description("라이딩 지역"),
		fieldWithPath("zone.code").description("라이딩 지역 코드"),
		fieldWithPath("zone.name").description("라이딩 지역 명칭"),
		fieldWithPath("bicycles[]").description("라이딩에서 허용하는 자전거 목록"),
		fieldWithPath("estimatedTime").description("라이딩 예상 소요 시간"),
		fieldWithPath("ridingCourses[]").description("라이딩 코스")};
	...
	void updateUserTest() throws Exception {
		...
		var riding = new RidingPostBriefInfo(1L, "한강 라이딩 하실분",
			...
			INTERMEDIATE.name(), LocalDateTime.now().plusDays(2L), new Coordinate(37.660666, 126.229333),
			new RidingPostBriefInfo.ZoneInfo(5, "서울시"),
			Collections.emptyList(), "1시간 30분", List.of("출발점", "도착점"));

		var ridingEnded = new RidingPostBriefInfo(1L, "한강 라이딩 하실분",
			"https://programmers.co.kr/assets/icons/apple-icon-6eafc2c4c58a21aef692d6e44ce99d41f999c71789f277317532d0a9c6db8976.png",
			INTERMEDIATE.name(), LocalDateTime.now().minusDays(4L), new Coordinate(37.660666, 126.229333),
			new RidingPostBriefInfo.ZoneInfo(5, "서울시"),
			Collections.emptyList(), "1시간 30분", List.of("출발점", "도착점"));

		UserProfilePageInfo result = UserProfilePageInfo.from(
			user, ParticipatedRidingInfo.from(List.of(riding), List.of(ridingEnded), List.of(riding),
				List.of(ridingEnded)));
		...
		mockMvc.perform(RestDocumentationRequestBuilders.get("/api/v1/users/{userId}", 1L)
			...
			.andDo(document("user-get",
				...
				responseFields(
					...
					fieldWithPath("ridings").description("해당 유저 관련 라이딩 정보"),
					fieldWithPath("ridings.leading[]").description("유저가 개설했던 라이딩"),
					fieldWithPath("ridings.finished[]").description("유저가 완료한 라이딩"),
					fieldWithPath("ridings.scheduled[]").description("유저가 참가할 예정인 라이딩"),
					fieldWithPath("ridings.canEvaluated[]").description("유저가 평가할 수 있는 라이딩"))
					.andWithPrefix("ridings.leading[].", ridingDescription)
					.andWithPrefix("ridings.finished[].", ridingDescription)
					.andWithPrefix("ridings.scheduled[].", ridingDescription)
					.andWithPrefix("ridings.canEvaluated[].", ridingDescription)
				)
			)
			.andDo(print());
	}
